...
API_LOGIN_URL = "https://8db6-42-118-190-115.ngrok-free.app/api/login/"
API_REFRESH_URL = "https://8db6-42-118-190-115.ngrok-free.app/api/token/refresh/"
...
def refresh_access_token(refresh_token: str) -> str | None:
    try:
        ...
        if response.status_code == 200:
            ...
            if new_access_token:
                # Cập nhật file token.json với access token mới
                save_token(new_access_token, refresh_token)
                return new_access_token
        else:
            print("❌ Refresh token hết hạn hoặc không hợp lệ.")
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                refresh_token = data.get("refresh") or data.get("refresh_token")  # optional

                if not access_token:
                    messagebox.showerror("Lỗi", "Không nhận được access token từ server.")
                    continue

                if not refresh_token:
                    messagebox.showwarning("Cảnh báo", "Không nhận được refresh token từ server.")

                save_token(access_token, refresh_token or "")
...
if __name__ == "__main__":
    ...
    if tokens:
        ...
        # Thử gọi API kiểm tra access token còn sống không
        try:
            check = requests.get(
                "https://httpbin.org/bearer",
                headers={"Authorization": f"Bearer {access_token}"}
            )
            if check.status_code == 401 and refresh_token:
                print("🔁 Access token hết hạn, đang làm mới bằng refresh token...")
                access_token = refresh_access_token(refresh_token)
        except:
            pass

    if not access_token:
        access_token = request_login()

    if access_token:
        root = Tk()
        app = AutoPosterGUI(root, token=access_token)
        root.mainloop()
